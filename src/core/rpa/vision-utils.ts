...
## 微信桌面端布局（三栏式）
- **最左侧**：导航栏（绿色背景），从上到下：头像、聊天入口💬图标、联系人图标、收藏等
- **中间列**：聊天联系人列表
  - 顶部是搜索框和加号按钮
  - 下方是联系人列表，每个联系人项包含：头像（圆形）、名称、最新消息预览、时间
  - 关键特征：头像右上角有**红色圆形数字角标**（如"1","2","5","10","99+"）表示该联系人有未读消息
  - 没有未读消息的联系人，头像右上角没有红色数字角标
- **右侧**：聊天区域（当前对话内容）

## 你的职责
帮我框选以下两个区域，每个区域用 <bbox>x1,y1,x2,y2</bbox> 格式，坐标范围 0-1000：

1. 【聊天入口按钮区域】— 最左侧导航栏中的💬聊天按钮（绿色背景那一列），包含💬图标和红色角标

2. 【第一个有未读红点的联系人】— 在**中间列**的联系人列表中：
   - 从联系人列表的**最顶部开始**，逐个往下检查每个联系人
   - 重点查看每个联系人的**头像右上角**：是否有**红色圆形数字角标**
   - 找到**第一个**（从上往下）头像右上角有红色数字角标的联系人
   - 框选该联系人的**头像区域**（圆形头像，必须包含红色数字角标）
   - 如果**所有联系人头像右上角都没有红色数字角标**，则只返回第1个bbox（聊天入口）

## 重要判断规则
- 红色数字角标 = 红色圆形背景 + 白色数字（如"1","2","99+","..."）
- 绿色背景 ≠ 红色角标（不要混淆）
- 必须确认看到**红色**数字角标，不能仅凭位置猜测
- 第2个bbox的y坐标应该在中间列（大约在150-800之间），不是在最左侧导航栏`,
    targets: ['【聊天入口按钮区域】', '【第一个有未读红点的联系人】']
  },
  wework: {
...
- 中间消息列表：顶部搜索框，下方是联系人消息列表，头像右侧或右上角有未读红点
- 右侧聊天区

## 你的职责
帮我框选以下两个区域，每个区域用 <bbox>x1,y1,x2,y2</bbox> 格式，坐标范围 0-1000：
1. 【消息按钮区域】— 左侧导航栏中的消息按钮区域，包含按钮和红色角标
2. 【第一个有未读红点的消息项】— 在中间消息列表中，从头开始往下找，找到第一个头像有红色未读标记的消息项

注意：第2点必须选有红色未读标记的消息项，如果没有找到，只返回第1个bbox`,
    targets: ['【消息按钮区域】', '【第一个有未读红点的消息项】']
  }
}
